...
PLIST has the form (\"fontname\" :prop1 val1 :prop2 val2 ...)"
  (let* ((font (car plist))
         ...
         (font-props (dotemacs-mplist-remove
                      (dotemacs-mplist-remove props :powerline-scale)
                      :powerline-offset))
    ...
    (when (and fallback-font-name fallback-font-name2)
      ...
      ;; for instance)
      (let* ((fallback-props (dotemacs-mplist-remove
                              (dotemacs-mplist-remove font-props :size)
                              :height))
