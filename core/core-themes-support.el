...
(defface org-kbd
  ...
  "Face for displaying key bindings in emacs documents."
  :group 'org-faces)

(defconst dotemacs-theme-name-to-package
  '(
...
(defvar dotemacs-used-theme-packages nil
  "List of packages of used themes.")

(defun dotemacs-get-theme-package (theme)
  "Returns the package theme for the given THEME name."
  (cond
   ;; built-in
   ((memq theme emacs-built-in-themes) nil)
   ;; from explicit alist
   ((assq theme dotemacs-theme-name-to-package)
    (cdr (assq theme dotemacs-theme-name-to-package)))
...
(defun dotemacs-load-theme (theme)
  ...
  (when (or (eq 'zonokai-blue theme)
        ...
        (dotemacs-load-or-install-package 'dash))
    ...
    (cond
     ...
     ((assq theme dotemacs-theme-name-to-package)
      (let* ((pkg (dotemacs-get-theme-package theme))
             (pkg-dir (dotemacs-load-or-install-package pkg)))
...
(defun dotemacs-cycle-dotemacs-theme ()
  ...
  (when dotemacs--cur-theme
    ...
    (setq dotemacs--cycle-themes
          (or (cdr (memq dotemacs--cur-theme dotemacs-themes))
              dotemacs-themes)))
...
(defadvice load-theme (after dotemacs-load-theme-adv activate)
  "Perform post load processing."
  (let ((theme (ad-get-arg 0)))
    (setq dotemacs--cur-theme theme)
    (dotemacs-post-theme-init theme)))

(defun dotemacs-post-theme-init (theme)
  " Some processing that needs to be done when the current theme has been
changed to THEME."
  (interactive)
  (when (fboundp 'dotemacs-set-state-faces)
