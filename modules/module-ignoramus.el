...
(dotemacs-use-package-add-hook projectile
  :post-config
  (progn
    (defun dotemacs//projectile-ignored-directory-p (fcn directory)
    ...
    (advice-add 'projectile-ignored-directory-p
                :around 'dotemacs//projectile-ignored-directory-p)

    (defun dotemacs//projectile-ignored-file-p (fcn file)
      (let ((case-fold-search ignoramus-case-insensitive))
        (string-match-p ignoramus-boring-file-regexp file)))
    (advice-add 'projectile-ignored-file-p
                :around 'dotemacs//projectile-ignored-file-p)
...
(dotemacs-use-package-add-hook helm
  ...
  (progn
    ...
    (setq helm-grep-ignored-directories dotemacs/directory-basename-exact-names)))
...
(use-package ignoramus
  ...
  (progn
    ...
    ;; pathname."
    (setq dotemacs/file-basename-exact-names
          '(
    ...
    (setq dotemacs/directory-basename-exact-names
          '(
            ".coverage"                            ; python
            ".git"                                 ; git
            ".hg"                                  ; mercurial
    ...
    (setq ignoramus-file-basename-exact-names
          (append dotemacs/file-basename-exact-names
