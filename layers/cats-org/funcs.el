...
(defun cats//org-effort-sum (&optional skip-done clock-diff)
...
(defun org-dblock-write:cats-project-review (params)
          ...
          (org-map-entries
           (lambda ()
             (let* ((level (- (org-current-level) base-level))
                    ...
                    (effort (cats//org-effort-sum))
                    (left (cats//org-effort-sum t t))
                    plist)
               (setq plist (list
                 :title title
